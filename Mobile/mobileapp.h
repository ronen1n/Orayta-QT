...
public:
    ...
    void downloadNext();
    void performDownloadNext();
    ~MobileApp();
    
    // Handle Android back button - returns true if handled, false to use default behavior
...
private slots:
    ...
    void on_deleteBooksBTN_clicked(bool checked);
    ...
    void showBook(int);
    void performShowBook(Book *, BookIter);
    ...
    void on_autoInterfaceSizeCKBX_clicked(bool checked);
    ...
    void on_toolbarStyleComboBox_currentIndexChanged(int index);

    void copyAssetsToDisk();
    void performAssetCopy();

    void on_pushButton_clicked();

    void on_menuBTN_clicked();

    void applyToolbarStyle(int style);
    void updateFontPreview();
    
    // Settings preview methods
...
private:
    ...
    // Helper methods for book removal
    void removeBookOrCategory(Book *b);
    int countBooksInCategory(Book *category);
    int removeBooksInCategory(Book *category);
    QMessageBox* createStyledMessageBox(QMessageBox::Icon icon, const QString &title, const QString &text);
    
    // Helper methods for saving/restoring tree state
    QSet<QString> getExpandedCategories();
    void restoreExpandedCategories(const QSet<QString> &expandedPaths);
...
//    #define KUK_SAVEDBOOKLIST   TMPPATH + "Kuk-Books"
    ...
    FileDownloader *downloader;  // Legacy - kept for compatibility
...
//    QWidgetList *viewHistory;
    ...
    // Settings preview - store original values before preview
    struct SettingsBackup {
        ...
        bool nightMode;
        int interfaceSize;
        bool autoInterfaceSize;
        int toolbarStyle;
        QString language;
    } originalSettings;
    
    // Track swipe gestures for tab navigation
    QPoint tabSwipeStartPos;
    bool tabSwipeInProgress;
    
    // Track scroll direction for tree widget
    QPoint treeScrollStartPos;
    bool isVerticalScrolling;
    
    // Track scroll for selection area to prevent accidental clicks
    QPoint selectionScrollStartPos;
    bool selectionAreaScrolling;
    
    // Track scroll for text displayer to prevent accidental chapter clicks
    QPoint textScrollStartPos;
    bool textDisplayerScrolling;

#ifdef ANDROID
    Android16Support *android16Support;
    EdgeToEdgeSupport *edgeToEdgeSupport;
