    ...
    if(link.indexOf("!") != -1 )
    {
        ...
        if(ToNum(parts[0], &id))
        {
            ...
            if( book )
            {
                ...
                // Validate the iterator before displaying
                if (!itr.isEmpty()) {
                    /*
                    if (parts.size() == 3)
                    ...
                    */
                    ...
                    // qDebug() << "Empty chapter iterator, ignoring click";
                }
            } else {
                // qDebug() << "Book not found for id:" << id;
            }
        }
    }
    //Link to website
...
void textDisplayer::display(Book * book, BookIter itr)
{
    ...
    if (u != QUrl()) {
        ...
        // qDebug()<< "Render error for chapter:" << itr.toString() << "in book:" << book->getName();
        // Don't change the current display if there's an error
