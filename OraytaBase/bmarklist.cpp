...
{
    // Save the time and position when press starts
...
void BMarkList::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    // Check if finger moved significantly (indicates scrolling, not clicking)
    int distance = (e->pos() - pressPos).manhattanLength();
    int delta = pressTime.msecsTo(QTime::currentTime());
    
    // If moved more than 10 pixels, it's a scroll gesture, not a click
    if (distance > 10) {
        QListWidget::mouseReleaseEvent(e);
        return;
    }
    
    // Distinguish between short press (tap) and long press
