...
void Book::BuildSearchTextDB()
{
    ...
    if (mFiletype == Book::Normal || mFiletype == Book::Html)
    {
        ...
        else if (mFiletype == Book::Html)
        {
            ...
            // Qt6: Use QStringConverter instead of QTextCodec
        ...
        for (int i=0; i<text.size(); i++)
        {
            ...
            else
            {
                ...
                text[i].replace(QRegularExpression(spaceSigns), " ");
...
QList <SearchResult> Book::findInBook(const QString& phrase)
{
    ...
    QRegularExpression regexp = QRegularExpression(phrase);
    return findInBook(regexp);
}

#define CHAR_LIMIT 250
QList <SearchResult> Book::findInBook(const QRegularExpression& exp)
{
    //qDebug() << "searching in: " << this->getNormallDisplayName();

    QList <SearchResult> results;
    QRegularExpression regexp = QRegularExpression("(" + exp.pattern()+ ")");

    if (pureText() == "") qWarning() << this->getNormallDisplayName() << " has no Search DB, so it couldn't be searched!";

    int curr = 0, last = 0;
    QRegularExpressionMatch match;
    while ((match = exp.match(pureText(), last)).hasMatch())
    {
        curr = match.capturedStart();
...
QString Book::resultPreview(const QRegularExpression& exp, int offset)
{
