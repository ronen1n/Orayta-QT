...
void BookList::addAllBooks (QString dirpath, bool isUserBooks, int parentindex)
{
    ...
    for (int i=0; i<list.size(); i++)
    {
        ...
        if ( ft != Book::Unkown )
        {
            ...
            if (!isUserBooks)
                Name.replace(QRegularExpression("^[0-9 ]*"), "").replace("_", " ").replace(".obk", "");
            ...
            {
                //Add confs for this book
                AddBookConfs(b, readfile(b->getPath().replace(QRegularExpression("\\.\\w{3,4}$"),".conf"), "UTF-8").split("\n"));
            }
    ...
    for (int i=0; i<list.size(); i++)
    {
        ...
        if ( ft != Book::Unkown )
        {
            ...
            QString Name = list[i].fileName();
            if (!isUserBooks)
                Name.replace(QRegularExpression("^[0-9 ]*"), "").replace("_", " ");
            ...
            {
                //Add confs for this book
                AddBookConfs(b, b->getPath().replace(QRegularExpression("\\.\\w{3,4}$"),".conf"));
            }
...
void BookList::AddBookConfs(Book *book, QList<QString> text)
{
    ...
    for(int i=0; i<text.size() ; i++)
    {
        ...
        {
            book->setNormallDisplayName(text[i].mid(12).replace(QRegularExpression("^ *"), ""));
        }
        ...
        else if (text[i].indexOf("MixedSources") != -1)
        {
            ...
            QList<int> wsrc;
            QStringList srcId = srcLst.split(',', Qt::SkipEmptyParts);
