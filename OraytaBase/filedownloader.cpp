...
void FileDownloader::downloadDone()
{
    if (cdownload->error())
    {
        qDebug() << "Download error:" << cdownload->errorString();
        qDebug() << "Error code:" << cdownload->error();
        ...
        QFile partFile(mFileName + ".part");
        if (partFile.exists())
        {
            partFile.remove();
            qDebug() << "Removed partial file after error:" << mFileName + ".part";
        }
    ...
    else
    {
        ...
        {
            // File is invalid (wrong hash or empty), clean up
            QFile partFile(mFileName + ".part");
            if (partFile.exists())
            {
                partFile.remove();
                qDebug() << "Removed invalid partial file:" << mFileName + ".part";
            }
            emit downloadError();
        }
    }
...
//Stops the download, removes the ".part" file
void FileDownloader::abort()
{
    if (cdownload)
    {
        qDebug() << "Aborting download:" << mFileName;
        cdownload->abort();
        cdownload->deleteLater();
        cdownload = NULL;
    }
    ...
    {
        partFile.remove();
        qDebug() << "Removed partial download file:" << mFileName + ".part";
    }
}
