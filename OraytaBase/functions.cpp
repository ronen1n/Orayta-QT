                     ...
                     QList <QString>& text, const char* encoding_name, bool skipconflines, bool encrypted)
{
    ...
    QTextStream t( &zfile );
    SET_TEXTSTREAM_CODEC(t, QTextCodec::codecForName(encoding_name));
...
void writetofile(QString filename, const QString& data, const char* encoding_name, bool overwrite)
{
    ...
    SET_TEXTSTREAM_CODEC(outstream, QTextCodec::codecForName(encoding_name));
...
QString removeNikud( const QString& str )
{
    ...
    QRegularExpression regexp(QString("[") + QChar(0x05B0) + "-" + QChar(0x05BC) +
...
QString removeTeamim(const QString& str)
{
    ...
    QRegularExpression regexp(QString("[") + QChar(0x0591) + '-' +  QChar(0x05AF) +
...
QRegularExpression withNikudAndTeamim( const QString& str )
{
    return QRegularExpression("(" + allowNikudAndTeamim(str) + ")");
}
...
void GenerateSearchTextDB(QString infile,  QString pureTextOutPath, QString levelMapOutPath)
{
    ...
    //Set all QString to work with unicode (Qt6: No-op, always UTF-8)
    ...
    QRegularExpression notText("[^א-תA-Za-z0-9 ]");
    //These are turned into spaces, and not just ignored.
    QRegularExpression spaceSigns("[-_:׃.,;?]");
    ...
    for (int i=0; i<text.size(); i++)
    {
        ...
        else
        {
            ...
            if ( infile.contains("מקרא") )
            {
                ...
                text[i].replace( QRegularExpression ("[(][^)]*[)] [[]([^]]*)[]]"), "\\1");
            }

            //Remove html tags
            text[i].replace( QRegularExpression("<[^>]*>"), "" );
...
//    writetofile(levelMapOutPath, lvlmpstr, "UTF-8", true);
    ...
    //IZAR: create OBK -----------------------
    QString outZipPath= infile.replace(QRegularExpression("\\.(txt|TXT)$"), ".obk");
    ...
    QString comment= readfile(infile.replace(QRegularExpression("\\.(txt|TXT)$"),".conf"), "UTF-8");
