...
QUrl SearchInBooks (const QRegularExpression& regexp, QString disp, vector<Book*> searchList, QProgressBar *pbar)
{
    ...
    if ( regexp.pattern() != "" )
    {
        ...
        else
        {
            ...
            {
                Htmlhead += "(" + QObject::tr("Showing first %1 results only").arg(results) + ")<BR><BR>";
            }
            else
            {
                Htmlhead += "<BR>";
