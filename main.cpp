...
// #include "Desktop/about.h"  // Temporarily disabled for Qt 5.15 verification
#define VERSION "0.081"  // Temporary definition for Qt 5.15 verification

// Global font variables - declared extern, defined in UI modules
#ifndef MOBILE
...
void initLang(QApplication *app)
{
    ...
    if (!translator->load(LANG + ".qm", ".")) {
        if (!translator->load(LANG + ".qm", transPath)) {
            qDebug() << "Failed to load translation:" << LANG;
        }
    }
...
{
#ifdef Q_OS_ANDROID
    // Force software rendering on Android to prevent OpenGL/accessibility deadlock crashes
    // This is a workaround for Qt 6.10 Android issues with comboboxes and popups
    ...
    // Set application-wide RTL layout for Hebrew text
...
#ifndef MOBILE
    // Desktop UI disabled - requires Qt5→Qt6 migration
    // For now, just test that the core library builds correctly
    qDebug() << "Orayta Windows build successful!";
    qDebug() << "Version:" << VERSION;
    qDebug() << "Book path:" << BOOKPATH;
    qDebug() << "User path:" << USERPATH;
    qDebug() << "Temp path:" << TMPPATH;
    qDebug() << "";
    qDebug() << "Note: Desktop UI requires Qt5→Qt6 migration:";
    qDebug() << "  - QRegExp → QRegularExpression";
    qDebug() << "  - QtWebKit → QtWebEngine";
    qDebug() << "";
    qDebug() << "Core library is working. Exiting...";
    return 0;
#endif
